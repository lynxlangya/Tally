...
- 步骤
  1. 新增入口：从 Home FAB 进入 QuickEntry 第一步（选分类，对照 `demos/justone_quick_entry_1`）
     - 打开方式：点击加号按钮展示系统 `sheet`（高度约 屏幕 2/3，顶部圆角：约 28~32，带轻微描边与阴影）
     - 顶部标题“选择分类”，右侧支/收切换胶囊
  2. 分类选择弹框 UI 细节
     - 背景：深色半透明卡片（`surface` 0.4~0.6）+ 顶部分割线
     - 分类网格：4 列；图标容器 56~60，圆角 14~16
     - 文案：分类名小号、偏灰；选中/hover 变亮
     - 滚动：仅内容区可滚动，隐藏滚动条
     - 交互：点击分类 -> 进入金额页；支/收切换刷新分类列表
  3. 数据逻辑
     - 数据源：CategoryRepository
     - 过滤：按类型（支出/收入）且 `isSystem` 允许显示
     - 排序：系统预设优先 + 用户分类按 sortOrder
     - 未分类可显示或置底（与分类管理保持一致）
  4. 选择分类后进入金额键盘页（对照 `demos/justone_quick_entry_2`）
     - 弹框高度从 2/3 增加（约 0.8~0.85 屏高）
     - 顶部展示：分类 Icon + 分类名 + 备注输入框
     - 备注输入框：调用系统键盘，最多 6 个字（超出自动截断）
  5. 金额输入区
     - 大号金额展示（含光标闪烁）
     - 自定义数字键盘：1~9、0、00、小数点、删除、日历入口
     - 金额格式：仅 CNY，两位小数；负数禁止
  6. 确认按钮
     - 底部“确认记账”主按钮，点击保存并关闭弹框
  7. 编辑模式：从 BillsList 进入并预填数据（可沿用同一 UI）
...
### 5.4 首页占位列表联动（新增/编辑后刷新）

- 步骤
  1. 首页列表数据改为真实账单
     - 数据来源：BillRepository / BillService（禁止 View 直接访问 CoreData）
     - 过滤：仅显示 `deletedAt == nil`
     - 排序：按发生时间倒序
  2. 新增联动
     - Home FAB -> QuickEntry -> 保存成功
     - 保存完成后触发 Home 列表刷新（如：关闭 sheet 时 `reload()`）
  3. 编辑联动
     - 点击首页列表 item：打开 QuickEntry（编辑模式）
     - 预填：分类、金额、备注、日期
     - 保存后刷新 Home 列表
  4. 列表交互
     - 左滑显示删除 Icon（使用 iOS `swipeActions` 样式）
     - 点击删除 -> 二次确认弹窗（仅确认 UI，不执行删除逻辑）
  5. UI 对齐
     - 复用 `JOListRow` 展示，保持首页风格与间距
