...
final class CSVImportPipelineTests: XCTestCase {
    ...
    func testDefaultImportExportServiceCSVPreviewAndImportFlow() async throws {
        ...
        """
        let fileURL = FileManager.default.temporaryDirectory.appendingPathComponent("justone-import-\(UUID().uuidString).csv")
    ...
    func testPreviewImportBackupCountsDuplicateBillIDAsConflict() async throws {
        ...
        """
        let fileURL = FileManager.default.temporaryDirectory.appendingPathComponent("justone-backup-\(UUID().uuidString).json")
    ...
    func testImportBackupRequiresManagedObjectContext() async throws {
        ...
        """
        let fileURL = FileManager.default.temporaryDirectory.appendingPathComponent("justone-backup-import-\(UUID().uuidString).json")
        try Data(payload.utf8).write(to: fileURL, options: .atomic)
        defer { try? FileManager.default.removeItem(at: fileURL) }
